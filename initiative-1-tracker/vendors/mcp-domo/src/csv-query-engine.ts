...
export class CSVQueryEngine {
   ...
   */
  private loadData(): { data: any[][]; columns: string[] } {
    ...
    const data = records.map((row: any) => columns.map((col) => row[col] ?? ''));
  ...
  async executeQuery(sql: string): Promise<QueryResult> {
    ...
    const resultData = filteredData.map((row) =>
      selectedColumnIndices.map((idx) => (idx >= 0 ? row[idx] : null))
    );
   ...
   */
  private evaluateWhereClause(whereClause: string, row: any[], columns: string[]): boolean {
   ...
   */
  getStats(): { rowCount: number; columnCount: number; lastModified: Date | null } {
    if (!this.hasCachedData()) {
